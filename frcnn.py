...
class FRCNN(object):
    ...
    def get_rpn_answer(self, image, crop = False, count = False):
        ...
        with torch.no_grad():
            ...
            #-------------------------------------------------------------#
            temp = torch.zeros_like(roi_cls_locs)
            results = self.bbox_util.forward(temp, roi_scores, rois, image_shape, input_shape, 
                                                    nms_iou = self.nms_iou, confidence = 0.5)
            #---------------------------------------------------------#
            #   如果没有检测出物体，返回原图
            #---------------------------------------------------------#           
            if len(results[0]) <= 0:
