...
from nets.frcnn_training import (FasterRCNNTrainer, RetinaNetTrainer, get_lr_scheduler,
                                 set_optimizer_lr, weights_init)
from nets.retinanet import RetinaNet
from utils.callbacks import EvalCallback, LossHistory
from utils.dataloader import FRCNNDataset, vocdataset, frcnn_dataset_collate, BatchCollator
from utils.utils import get_classes, show_config
from utils.utils_fit import fit_one_epoch_retina
from nets.data import transform,targettransform
...
if __name__ == "__main__":
    ...
    #---------------------------------------------------------------------#
    classes_path    = r'D:\1Acyn\FUDAN\2022学年\计算机视觉\pj2-目标检测算法\faster-rcnn-pytorch\model_data\voc_classes.txt'
    #----------------------------------------------------------------------------------------------------------------------------#
    ...
    #   获得图片路径和标签
    #----------------------------------------------------#
    train_annotation_path   = r'D:\1Acyn\FUDAN\2022学年\计算机视觉\pj2-目标检测算法\faster-rcnn-pytorch\2012_train.txt'
    val_annotation_path     = r'D:\1Acyn\FUDAN\2022学年\计算机视觉\pj2-目标检测算法\faster-rcnn-pytorch\2012_val.txt'
    ...
    if True:
        ...
        train_dataset = vocdataset(cfg, is_train=True, transform=transform(cfg,is_train=True),
                                target_transform=targettransform(cfg))

        val_dataset = vocdataset(cfg=cfg, is_train=False,
                                transform=transform(cfg=cfg, is_train=False),
                                target_transform=targettransform(cfg))
        
        gen = DataLoader(train_dataset, shuffle = True, batch_size = batch_size, num_workers = num_workers, pin_memory=True,
                                    drop_last=True, collate_fn=BatchCollator(is_train=train_dataset.is_train))
        gen_val = DataLoader(val_dataset  , shuffle = True, batch_size = batch_size, num_workers = num_workers, pin_memory=True, 
                                    drop_last=True, collate_fn=BatchCollator(is_train=True))

        train_util = RetinaNetTrainer(model_train, optimizer, cfg)
        #----------------------#
        #   记录eval的map曲线
        #----------------------#
        ...
        for epoch in range(Init_Epoch, UnFreeze_Epoch):
            ...
            fit_one_epoch_retina(model, train_util, loss_history, eval_callback, optimizer, epoch, epoch_step, epoch_step_val, gen, gen_val, UnFreeze_Epoch, Cuda, fp16, scaler, save_period, save_dir)
