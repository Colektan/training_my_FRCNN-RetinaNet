...
class RetinaNetTrainer(nn.Module):
    def __init__(self, model_train, optimizer, cfg):
        super(RetinaNetTrainer, self).__init__()
        self.model_train    = model_train
        self.optimizer      = optimizer
    ...
    def forward(self, imgs, bboxes, labels, scale):
        cls_logits, bbox_preds = self.model_train(imgs)
        reg_loss, cls_loss = self.loss_func(cls_logits, bbox_preds, labels, bboxes)

        reg_loss = reg_loss.sum()
        cls_loss = cls_loss.sum()
            
        losses = [reg_loss, cls_loss]
        losses = losses + [sum(losses)]
        return losses
